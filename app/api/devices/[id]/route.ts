...
export async function GET(
  ...
  try {
    ...
    const promptsToday = promptsData?.filter((p: any) => {
    ...
    const ipAddress = Array.isArray(data.ips) && data.ips.length > 0 ? data.ips[0] : 'N/A'
    
    // Count browsers from JSONB array
    const browserCount = Array.isArray(data.browsers) ? data.browsers.length : 0
